    ...
    return (
        <Container className="receipt-detail-wrapper">
            ...
            <div className="infomation-wrapper">
                <h2 className="information-field display-flex">Trạng thái: {"  "}
                    <p className={"receipt_status_" + receipt.status}>{receiptStatus[receipt.status]}</p>
                </h2>
            </div>
            ...
            <div className="infomation-wrapper">
                ...
                <p className="infomation-content">
                    ...
                    {`(${receipt?.receiver?.phone}) \n ${receipt.receiver?.description ? receipt.receiver?.description : ""} ${receipt.receiver?.ward.name} ${receipt.receiver?.district.name} ${receipt.receiver?.province.name}`}
                </p>
            </div>
...

        {
            ...
            <button
                ...
                onClick={() => { dispatch(appActions.openConfirmDialog('Xác nhận hủy đơn hàng?', () => handleCancelOrder())) }}> Hủy đơn hàng
            </button>
        }
        </Container>
    );
