...
function UserInfo() {
    ...
    const userSchema =
        Yup.object().shape({
            ...
            birthday: Yup.date()
                .max(new Date(maxDate), "Your age must be than 18")
        })
    const initialValues = {
        ...
        <div className='user-info__wrapper'>
            ...
            <Formik
                ...
                onSubmit={values => {
                    ...
                    dispatch(appActions.openConfirmDialog("Lưu thay đổi?", () => handleSubmit(values)));
                }}
            >
                {({ errors, touched, resetForm }) => (
                    <Form className='form-info'>
                        <div className='info-item'>
                            ...
                            <input value={currentUser?.email} id={'info-item__email'} disabled={true} />
                        </div>
                        {
                            inputList.map((val, idx) => (
                                <div key={idx} className='info-item'>
                                    ...
                                    {
                                        val.type !== 'select' ? <Field name={val.name} type={val.type} id={'info-item__' + val.name} />
                        ...
                        <div className='save-reset-password__btn-group'>
                            <button className='btn-save' type='submit'>Lưu thông tin</button>
                            <button onClick={resetForm} type='button'>Bỏ thay đổi</button>
                        </div>
                    </Form>
                )}
